 ...
 */
async function exportGitPatch(outputDir) {
    ...
    // ---- Step 1: 获取当前分支名 ----
    const branchResult = await (0, shell_1.execCommand)('git rev-parse --abbrev-ref HEAD', execOpts);
    ...
    if (isClean) {
        ...
        const addResult = await (0, shell_1.execCommand)('git add .', execOpts);
        log.detail('exitCode', String(addResult.exitCode));
        if (addResult.stderr) {
            log.detail('stderr', addResult.stderr.trim());
        }
        logs.push({
            step: 'git add .',
            command: 'git add .',
        ...
        const commitResult = await (0, shell_1.execCommand)(`git commit -m "${commitMsg}"`, execOpts);
        logs.push({
            step: 'git commit',
        ...
        if (commitResult.stdout) {
            log.detail('stdout', commitResult.stdout.trim());
        }
        if (commitResult.stderr) {
            log.detail('stderr', commitResult.stderr.trim());
        }
    ...
    const diffResult = await (0, shell_1.execCommand)('git diff main...HEAD', execOpts);
    logs.push({
        step: '生成 diff patch',
    ...
    // ---- Step 7: 写入 patch 文件 ----
    await (0, fileUtils_1.ensureDir)(outputDir);
