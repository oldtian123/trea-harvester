...
class HubRegistry {
    ...
    async callWindow(sessionId, tool, args = {}) {
...
const PORT = Number(process.env.TRAE_HUB_PORT) || protocol_1.DEFAULT_HUB_PORT;
function log(msg) {
    console.log(`[Hub ${new Date().toISOString()}] ${msg}`);
}
const mcpDeps = {
    listWindows: () => registry.list(),
    callWindow: (sessionId, tool, args) => registry.callWindow(sessionId, tool, args),
};
...
app.use(protocol_1.MCP_PATH, (req, res, next) => {
...
const mcpServer = (0, mcpTools_1.createHubMcpServer)(mcpDeps);
mcpServer.connect(statelessTransport);
app.post(protocol_1.MCP_PATH, async (req, res) => {
    let body = '';
...
server.listen(PORT, '127.0.0.1', () => {
    STARTED_AT = Date.now();
    const info = { port: PORT, token: AUTH_TOKEN, pid: process.pid, startedAt: STARTED_AT };
    try {
        fs.writeFileSync(protocol_1.HUB_INFO_FILE, JSON.stringify(info, null, 2), 'utf-8');
    }
    catch (e) {
    ...
    log(`   MCP endpoint: http://127.0.0.1:${PORT}${protocol_1.MCP_PATH}`);
    // 启动端口扫描
